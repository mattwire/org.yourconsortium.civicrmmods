...
class CRM_Case_BAO_CaseContact extends CRM_Case_DAO_CaseContact {
  ...
  public static function create($params) {
    // Check if we have an existing CaseContact
    try {
      $caseContactResult = civicrm_api3('CaseContact', 'getsingle', array(
        'case_id' => $params['case_id'],
        'contact_id' => $params['contact_id'],
      ));
      $params['id'] = $caseContactResult['id'];
    }
    catch (CiviCRM_API3_Exception $e) {
      // No existing caseContact, so we'll create a new one
    }
